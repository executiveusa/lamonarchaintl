  ...
  return (
    <div className="space-y-8">
      <section className="bg-white rounded-lg border p-6">
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
          <div>
            ...
            <Input id="place-name" value={name} onChange={(e) => handleNameChange(e.target.value)} required />
          </div>
          <div>
            <Label htmlFor="place-slug">Slug</Label>
            <Input id="place-slug" value={slug} onChange={(e) => setSlug(e.target.value)} required />
          </div>
          <div>
            <Label htmlFor="place-neighborhood">Neighborhood</Label>
            <Input id="place-neighborhood" value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} />
          </div>
          <div>
          ...
          <div>
            <Label htmlFor="place-website">Official website</Label>
            <Input id="place-website" type="url" value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} />
          </div>
          <div>
            <Label htmlFor="place-instagram">Official Instagram URL</Label>
            <Input id="place-instagram" type="url" value={instagramUrl} onChange={(e) => setInstagramUrl(e.target.value)} />
          </div>
          <div className="md:col-span-2">
            <Label htmlFor="place-description-es">Editorial note — Spanish</Label>
            <Textarea id="place-description-es" value={descriptionEs} onChange={(e) => setDescriptionEs(e.target.value)} />
          </div>
          <div className="md:col-span-2">
            <Label htmlFor="place-description-en">Editorial note — English</Label>
            <Textarea id="place-description-en" value={descriptionEn} onChange={(e) => setDescriptionEn(e.target.value)} />
          </div>
          <div className="md:col-span-2">
            <Button type="submit" disabled={isSaving}>{isSaving ? 'Saving…' : 'Save to verification queue'}</Button>
          </div>
        </form>
      </section>
      ...
      <section className="bg-white rounded-lg border p-6">
        ...
        <p className="text-sm text-gray-600 mt-1">Verification needs evidence. This action records who verified the place and when.</p>
        <form onSubmit={handleVerify} className="space-y-4 mt-5">
          <div>
            ...
            <select
              ...
              onChange={(e) => setVerificationPlaceId(e.target.value)}
              className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm"
            >
              <option value="">Choose a place</option>
              {places.filter((place) => place.verification_status !== 'verified').map((place) => (
                <option key={place.id} value={place.id}>{place.name} — {place.verification_status}</option>
              ))}
            </select>
          </div>
          <div>
          ...
          <div>
            <Label htmlFor="verification-notes">Evidence / notes</Label>
            <Textarea
              id="verification-notes"
              value={verificationNotes}
              onChange={(e) => setVerificationNotes(e.target.value)}
              placeholder="What was checked? Who confirmed it? What evidence exists?"
              required
            />
          </div>
      ...
      <section className="bg-white rounded-lg border p-6">
        ...
        <div className="mt-4 space-y-3">
          ...
          {places.map((place) => (
            <div key={place.id} className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b pb-3">
              <div>
                <div className="font-medium">{place.name}</div>
                <div className="text-xs text-gray-500">{place.neighborhood || place.city}</div>
              </div>
              <div className="text-xs text-gray-600">
                verification: <strong>{place.verification_status}</strong> · publication: <strong>{place.publication_status}</strong>
              </div>
            </div>
          ))}
        </div>
